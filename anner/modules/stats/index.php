...
{
	echo("<b>Списание холдов:</b><br><br>");


$shold=mysql_query("SELECT `id` FROM `tables` WHERE `uid`='".$user_arr[0]."'");
...
unset($ror,$shold);
	...
	echo("<br><br><b>История операций:</b><br>");

	echo("<br><br><table class='ap_list_table'>
		<tr>
...
$perpage="10"; $get_array['pg']=intval($get_array['pg']); $rows=mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM `statistic` WHERE `uid`='".$user_arr[0]."'")); $num=ceil($rows[0]/$perpage); unset($rows);
if($get_array['pg']>$num OR $get_array['pg']<1 OR $get_array['pg']=="0") $get_array['pg']="1"; $start=intval($get_array['pg']*$perpage-$perpage);
	$site_arr=mysql_query("SELECT `id`,`tip`,`date`,`summa`,`balance` FROM `statistic` WHERE `uid`='".$user_arr[0]."' ORDER BY `id` DESC LIMIT $start,$perpage");
...
if($get_array['pg']>1) 
{ 
echo("<a class='ap_nav_block_a' href='stats.html&pg=$pagem'>Назад</a> ");
} 
while($i<=$num) 
{
if($get_array['pg']==$i) 
...
echo("<a class='ap_nav_block_a' href='stats.html&pg=$i'>$i</a> ");
if($get_array['pg']==$i) 
{ 
echo("</b>"); 
} 
...
if($get_array['pg']<$num) 
{ 
echo("<a class='ap_nav_block_a' href='stats.html&pg=$pagep'>Вперед</a>");
}
